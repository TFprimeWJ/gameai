...
class Board():
    ...
    def __init__(self, height=None, width=None, win_length=None, np_pieces=None):
        ...
        if np_pieces is None:
            self.np_pieces = np.zeros([self.height, self.width]) # 新建一个新的空的棋盘
        else:
            self.np_pieces = np_pieces
            assert self.np_pieces.shape == (self.height, self.width) # 载入一个已有的棋盘

    def add_stone(self, position, player):
        """
        Create copy of board containing new stone.
        position - tuple: (x, y)
        player - scalar: 1 or -1
        """
        if (self.np_pieces[position[0]][position[1]] != 0):
            raise ValueError("Can't play position(%s, %s) on board %s" % (position[0], position[1], self))
        self.np_pieces[position[0]][position[1]] = player # player是 1 或 -1

    def get_valid_moves(self):
    	"""Any zero value in position(x, y) is a valid move"""
    ...
    def get_win_state(self):
        ...
        if not self.get_valid_moves()[0].any():
    ...
    def _is_diagonal_winner(self, player_pieces):
        ...
        for i in range(len(player_pieces) - win_length + 1):
            for j in range(len(player_pieces[0]) - win_length + 1):
                if all(player_pieces[i + x][j + x] for x in range(win_length)): # all(): 是否全为1
                    return True
